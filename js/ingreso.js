...
async function enviarDatosAPI() {
    try {
        ...
        // Si hay un pedido seleccionado, marcarlo como RECIBIDO
        if (pedidoSeleccionado) {
            await marcarPedidoRecibido(pedidoSeleccionado);
        }
...
// Función para marcar el pedido como RECIBIDO
    ...
    try {
        ...
        const datosPedido = {
            ...
            pedido: {
                ...
                id_proveedor: pedido.ID_PROVEEDOR || "0", // Asumiendo que puede no estar presente
                motivo: "2", // 2 representa RECIBIDO
                responsable: userSession.nombre || userSession.username
            }
        };

        const response = await fetch('https://script.google.com/macros/s/AKfycbzkvqnflAjsYuXlmB4N7SRQjUQruEFmjtY57L-VC-bVTFKrek9B-Jrp-JdCp-M_FZcR/exec', {
            method: 'POST',
            headers: {
                'Content-Type': 'text/plain;charset=utf-8',
            },
        ...
        if (!response.ok) {
            throw new Error(`Error al marcar pedido: ${response.statusText}`);
        }

        return await response.json();
    } catch (error) {
        console.error('Error al marcar pedido como recibido:', error);
        // No mostrar alerta para no interrumpir el flujo principal
...
function seleccionarPedido(pedido) {
    // Guardar todos los datos del pedido para usarlos después
    pedidoSeleccionado = {
        ...pedido,
        NRO_PEDIDO: pedido.NRO_PEDIDO,
        SEDE: pedido.SEDE,
        ...
        PRESENTACION: pedido.PRESENTACION,
        CANTIDAD: pedido.CANTIDAD,
        ID_PROVEEDOR: pedido.ID_PROVEEDOR || "0" // Valor por defecto si no existe
    };
